...
- **IPSW (macOS)**: `PackerLiteTemplateResolver.resolve(...)` (`Sources/cakedlib/PackerLite/PackerLiteTemplateResolver.swift`) picks the template in this order, throwing if none apply: explicit `--template <path>` → macOS version auto-detected from the IPSW filename (`MacOSVersion.detect(fromIPSWFilename:)`, Apple's `UniversalMac_<version>_<build>_Restore.ipsw` convention, which also yields the dotted version string, e.g. "15.6") → explicit `--macos-version` (`monterey`/`ventura`/`sonoma`/`sequoia`/`tahoe`/`goldengate`) → failure. Built-in templates are bundled as `CakedLib` SPM resources at `Sources/cakedlib/PackerLite/Resources/vanilla-*.packerlite.yaml` (currently `sequoia`, `tahoe`, `monterey`, `ventura`, and `sonoma` — no `goldengate` template exists yet). The detected codename/version are persisted unconditionally into `CakeConfig.osName`/`osRelease` (regardless of `--autoinstall`), so a later standalone `caked provision` run knows what the VM is without the original IPSW.
...
**`PackerLiteEngine`** (`Sources/cakedlib/PackerLite/PackerLiteEngine.swift`) has two entry points: `provision(location:config:template:runMode:progressHandler:)` boots the VM headlessly, waits for an IP, drives it, and shuts it down — used by `VMBuilder`'s build-time path. `provision(vm:template:runningIP:runMode:progressHandler:)` just drives an already-running `VirtualMachine` — used by `CakedLib.ProvisionHandler` (`Sources/cakedlib/Handlers/ProvisionHandler.swift`), the shared standalone-provisioning entry point both `caked provision` and the `Provision` gRPC RPC delegate to (see below), which boots the VM itself (with a UI window locally, headlessly-but-with-VNC over gRPC) instead of assuming a caller already booted it. `provision` resolves non-macOS VMs the same way `VMBuilder` does — the VM's stored `CakeConfig.configuredPlatform`, `--template` to override, required only when the platform has no built-in default (macOS VMs likewise auto-resolve from the stored `osName`); either way it installs the cakeagent once an IP is available, since ISO Linux VMs that skipped cloud-init won't have it yet. Both `PackerLiteEngine.provision` entry points mark `CakeConfig.provisioned = true` on success (see below) via the shared driving path.
