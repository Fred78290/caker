...
### VM image catalog and `--<id>` shorthand flags

`Sources/cakedlib/Resources/VMImages.json` + `Sources/cakedlib/VMImageCatalog.swift` (`VMImageEntry`/`VMImageArchCatalog`/`VMImageCatalog`, both `public`) are the catalog of known VM images — arch-split (`arm64`/`amd64`), category-split (`iso`/`ipsw`/`cloud`), keyed by short ids like `macos12`, `ubuntu2604`, `ubuntu2604Desktop`, `fedora44Server`, `centos9`. This used to live in the `caker` GUI target only (`Sources/caker/VMImageCatalog.swift`, reading `Sources/caker/Resources/VMImages.json`) for the wizard's image picker; it moved into `CakedLib` so `caked`/`cakectl` — which don't depend on `caker` — can resolve catalog ids too, for `build`/`launch`'s `--<id>` shorthand flags (below). `caker` still uses it the same way it always did (`VMImageCatalog.shared`), just via `CakedLib` now. It follows the same `Bundle.module`-vs-Xcode-native-target `#if SWIFT_PACKAGE ... #else ...` dance as `PackerLiteTemplateResolver.swift` (see the "`Bundle.module` gotcha" note above) — reuse that pattern, don't reinvent it, if you touch resource loading here. `<CAKE_HOME>/VMImages.json`, if present, overrides the bundled catalog (see `VMImageCatalog.refreshFromGitHub()`); `VMImageCatalog.githubCatalogURL` points at the file's current repo path and must be kept in sync if the file ever moves again.

`Sources/Grpc/options/VMImageShorthandFlags.swift` is a **generated file** — one `@Flag(name: .customLong("<id>"))` boolean per catalog id (e.g. `--macos12`, `--ubuntu2604Desktop`), regenerated from `VMImages.json` by:

```bash
python3 Scripts/generate-vm-image-shorthand-flags.py
```

Run this manually whenever `VMImages.json`'s set of ids changes (add/rename/remove) — it's not wired into the build as a plugin. The flags live on their own `VMImageShorthandFlags: ParsableArguments` type, not as a `public extension BuildOptions { ... }` adding `@Flag` vars directly — Swift doesn't allow stored properties (which a property-wrapper-backed property is, under the hood) in an extension of a struct, so `BuildOptions` (hand-written) composes it with one `@OptionGroup public var imageShorthand: VMImageShorthandFlags` line instead. `webui/scripts/sync-vm-images.mjs` is the precedent this generator mirrors (same source JSON, same "regenerate manually, commit the output" model).

**`@OptionGroup`/`@Flag` gotcha — `imageShorthand` is only safe to read on a really-parsed instance, so its ambiguity check lives in `validate()`, not `validate(remote:)`.** ArgumentParser's own internal validators (`UniqueNamesValidator`, `NonsenseFlagsValidator`, `CodingKeyValidator`, ...) build `--help`/parsing metadata by calling `Mirror(reflecting: Type.init())` on every `ParsableArguments` type reachable from a command — recursively including `VMImageShorthandFlags` via `BuildOptions`'s `@OptionGroup`. Every `@Argument`/`@Option`/`@Flag`/`@OptionGroup` property must still be in its unresolved "definition" state when that reflection runs; resolving any of them early (assigning a value from a custom initializer, or eagerly writing `false` defaults into `VMImageShorthandFlags.init()`) crashes that walk with "Trying to get the argument set from a resolved/parsed property." The corollary: `imageShorthand` — and any `VMImageShorthandFlags()` instance in general — is only safe to *read* after real ArgumentParser parsing (an actual `caked build --macos12 ...` invocation, or `BuildOptions.parse([...])` in a test); reading it on one built via `BuildOptions()`/`BuildOptions(request:)`/the `name:diskFormat:` convenience initializer crashes the *other* way ("Can't read a value from a parsable argument definition"). None of `BuildOptions`'s hand-written initializers touch `imageShorthand` at all, for exactly this reason.

This mattered concretely because `BuildOptions.validate(remote:)` (this codebase's own convention, distinct from the protocol) is *also* called on non-CLI-parsed instances — `CakerEnvVM.toBuildOptions(name:)` (`Sources/cakedlib/CakerEnv.swift`, used by `caked`/`cakectl up` for `.cakerenv` files) builds a `BuildOptions` via the `name:diskFormat:...` convenience initializer and hands it straight to `validate(remote:)`, with no real parsing involved. So the shorthand-flag ambiguity check and `imageId` population instead live in `BuildOptions.validate() throws` — `ParsableArguments`'s actual zero-argument protocol hook, which ArgumentParser guarantees only ever runs on an instance it just decoded (automatically, via `OptionGroup.init(from:)`'s `try wrappedValue.validate()`, for every real `caked build`/`cakectl build`/`launch` invocation and every `BuildOptions.parse([...])`) — never on one built any other way. `validate(remote:)` no longer touches `imageShorthand`/`imageId` at all.

`BuildOptions.imageId: String?` is the actual value that crosses the `GRPCLib`/`CakedLib` boundary — a plain `String?` with no ArgumentParser storage behind it, always safe to read/write from anywhere, unlike `imageShorthand`. It's populated either by `validate()` (from exactly one `imageShorthand` flag, for a local `caked build --macos12`) or decoded straight off the wire (`Caked_CommonBuildRequest.imageId` → generated Swift property `imageID`, since swift-protobuf capitalizes `Id`/`Url`-style suffixes regardless of the proto field's own casing — for `cakectl build --macos12`, which resolves the flag to an id client-side during its own parse and sends just the id rather than 53 booleans). `Sources/grpc/CakedAliases.swift`'s `Caked_CommonBuildRequest.init(buildOptions:)` bridges `imageId` onto the wire the same way it already does for `provisionTemplate`/`provisionVars`/`macosVersion`.
