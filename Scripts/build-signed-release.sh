...
BUILDDIR="${PROJECT_ROOT}/.build/universal/release"
...
for FILE in Caker caked cakectl; do
	lipo -create "${PROJECT_ROOT}/.build/x86_64-apple-macosx/release/${FILE}" "${PROJECT_ROOT}/.build/arm64-apple-macosx/release/${FILE}" -output "${BUILDDIR}/${FILE}"
done
