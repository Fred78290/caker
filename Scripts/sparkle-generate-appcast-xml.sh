...
get_releases_data() {
    ...
    if ! gh api "/repos/$GITHUB_REPO/releases" \
        --paginate \
    ...
    if [ -f "$sig_file" ]; then
        ...
        if [ -f "$PROJECT_ROOT/.sparkle/sparkle_private_key.pem" ] && [ -f "$dmg_path" ]; then
            if command -v generate_appcast &> /dev/null; then
                generate_appcast --ed-key-file "$PROJECT_ROOT/.sparkle/sparkle_private_key.pem" --download-url-prefix "" --output-file "/tmp/temp_appcast.xml" "$dmg_path" 2>/dev/null | grep 'sparkle:edSignature' | sed 's/.*sparkle:edSignature="\([^"]*\)".*/\1/' || echo ""
            fi
        fi
    fi
}
...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" xmlns:dc="http://purl.org/dc/elements/1.1/">
    <channel>
        ...
        <docs>https://fred78290.github.io/caker/</docs>
...
generate_release_item() {
    ...
    cat << EOF
        <item>
            <title>$name</title>
            <link>https://github.com/$GITHUB_REPO/releases/tag/$tag_name</link>
...
display_summary() {
    ...
    echo -e "🔗 URL: ${GREEN}https://fred78290.github.io/caker/appcast/appcast.xml${NC}"
    echo
    echo -e "${YELLOW}💡 Next steps:${NC}"
    echo "1. Update Info.plist to use custom appcast:"
    echo "   SUFeedURL = https://fred78290.github.io/caker/appcast/appcast.xml"
