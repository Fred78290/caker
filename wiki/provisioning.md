...
**Linux** : sept templates sont également fournis en ressources embarquées, sous les mêmes noms de fichiers `linux-*.packerlite.yaml` : `linux-fedora` (Fedora Workstation, Anaconda — ISO Live, sélectionné quand l'URL/nom de fichier contient « workstation » ou « desktop »), `linux-fedora-server` (Fedora Server, Anaconda — même flux qu'un ISO CentOS/RHEL, boot direct sans session live ; sélectionné par défaut sinon), `linux-centos` (CentOS Stream, Anaconda), `linux-redhat` (RHEL, Anaconda — `redhat` **et** `rhel` sont tous deux reconnus dans le nom de fichier), `linux-opensuse` (openSUSE Leap, installeur YaST), `linux-debian` (Debian, debian-installer) et `linux-alpine` (Alpine, `setup-alpine`). La plupart ont depuis été réécrits pour utiliser une navigation synchronisée par OCR (`<locate>`) plutôt que des clics/délais fixes, au fil de plusieurs passes de correction visant explicitement à coller à ce que l'installeur affiche réellement — contrairement à leur état d'origine (relu pour la plausibilité seulement), cela suppose une itération contre de vrais démarrages, même si aucun commit ne revendique une validation de bout en bout complète ; attendez-vous malgré tout à devoir ajuster ponctuellement des cibles de clic. Le template `linux-opensuse` cible l'installeur YaST (openSUSE Leap ≤ 15.6) — les ISO Leap 16.0/16.1 récemment ajoutées à `VMImages.json` utilisent le nouvel installeur Agama, non couvert par ce template ; fournissez votre propre `--template` pour ces versions en attendant. Ubuntu n'a pas de template PackerLite (utilise cloud-init/subiquity) ; toute autre distribution nécessite un `--template` personnalisé.
...
| `<skipStepIfNotFound text='...' steps=N timeout=N>` | Comme `<skipNotFound>`, mais au lieu d'abandonner le reste du bloc `title` courant si le texte n'apparaît pas dans le délai, saute exactement les `N` étapes suivantes (par défaut 1) puis continue normalement. Plus précis que `<skipNotFound>` quand seules quelques étapes d'un écran sont conditionnelles. |
...
`<click text=...>`, `<locate>`, `<skipNotFound>` et `<skipStepIfNotFound>` acceptent aussi une liste d'alternatives séparées par `|` dans `text=`, ex. `text="Accept|Continue"` — l'étape correspond si l'une des deux chaînes est reconnue à l'écran, utile pour un texte dont le libellé varie selon la version/la langue de l'installeur.
...
| `<skipStepIfNotFound text='...' steps=N timeout=N>` | Like `<skipNotFound>`, but instead of abandoning the rest of the current `title` block when the text isn't found in time, skips exactly the next `N` steps (default 1) and then continues normally. More precise than `<skipNotFound>` when only a couple of a screen's steps are conditional. |
...
`<click text=...>`, `<locate>`, `<skipNotFound>`, and `<skipStepIfNotFound>` all also accept a pipe-delimited list of alternatives in `text=`, e.g. `text="Accept|Continue"` — the step matches if either string is recognized on screen, useful for wording that varies across installer versions/locales.
