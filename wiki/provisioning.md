...
**Variables de provisioning mémorisées** : `--var` (en CLI, `caked build`/`provision` et `cakectl build`/`provision`) et l'éditeur « Provisioning variables » du wizard de création de VM de Caker.app (affiché sous le sélecteur d'image ISO/IPSW) partagent le même magasin persistant, `<CAKE_HOME>/ProvisionVariables.json` — toute variable ajoutée d'un côté est automatiquement disponible de l'autre, sans avoir à la ressaisir à chaque VM. Si aucun `--var` n'est fourni, les variables mémorisées sont utilisées telles quelles ; si un `--var` est fourni et qu'une de ses clés entre en collision avec une variable déjà mémorisée sous le même nom, la commande **échoue** avec une erreur « Duplicate provision variable » plutôt que de laisser l'un des deux l'emporter silencieusement — renommez la variable ou supprimez l'entrée mémorisée en conflit. Les noms/valeurs ne peuvent pas contenir `=` ni `|`.

### Templates intégrés

**macOS** : six templates sont fournis en ressources embarquées (`Sources/cakedlib/PackerLite/Resources/`), un par version — `macos12`, `macos13`, `macos14`, `macos15` (à l'origine transcrit depuis un template Packer `sequoia.pkr.hcl`), `macos26` (à l'origine transcrit depuis `tahoe.pkr.hcl`) et `macos27`. Ces anciens templates Packer/Tart de référence ont depuis été retirés du dépôt — PackerLite ne dépend plus d'eux. Toutes les six versions macOS reconnues ont désormais un template intégré. Les identifiants de version ont été renommés depuis les noms marketing d'origine (`monterey`→`macos12`, `ventura`→`macos13`, `sonoma`→`macos14`, `sequoia`→`macos15`, `tahoe`→`macos26`, `goldengate`→`macos27`) ; les anciens noms restent acceptés comme valeur de `--macos-version` par compatibilité.

**Linux** : neuf templates sont également fournis en ressources embarquées (les préfixes `linux-`/`vanilla-` ont depuis été retirés de tous les noms de fichiers embarqués, macOS compris) : `fedora` (Fedora Workstation, Anaconda — ISO Live, boot dans un bureau live puis assistant graphique de premier démarrage GNOME ; sélectionné quand l'URL/nom de fichier contient « workstation » ou « desktop »), `fedora-server` (Fedora Server, Anaconda — même flux qu'un ISO CentOS/RHEL, boot direct sans session live ; sélectionné par défaut sinon), `centos` (CentOS Stream, Anaconda), `redhat` (RHEL, Anaconda — `redhat` **et** `rhel` sont tous deux reconnus dans le nom de fichier), `opensuse` (openSUSE Leap, installeur YaST), `debian` (Debian, debian-installer), `alpine` (Alpine, `setup-alpine`), et `ubuntu`/`ubuntu-server` (Ubuntu Desktop et Server). La plupart ont depuis été réécrits pour utiliser une navigation synchronisée par OCR (`<locate>`) plutôt que des clics/délais fixes, au fil de plusieurs passes de correction visant explicitement à coller à ce que l'installeur affiche réellement — contrairement à leur état d'origine (relu pour la plausibilité seulement), cela suppose une itération contre de vrais démarrages, même si aucun commit ne revendique une validation de bout en bout complète ; attendez-vous malgré tout à devoir ajuster ponctuellement des cibles de clic. Le template `opensuse` cible l'installeur YaST (openSUSE Leap ≤ 15.6) — les ISO Leap 16.0/16.1 récemment ajoutées à `VMImages.json` utilisent le nouvel installeur Agama, non couvert par ce template ; fournissez votre propre `--template` pour ces versions en attendant. Ubuntu conserve aussi son propre mécanisme d'installation automatisée natif (cloud-init/subiquity, via l'option de démarrage « autoinstall ») — les templates `ubuntu*` sont une alternative optionnelle qui passe par la même approche PackerLite pilotée par OCR que toutes les autres distributions ; contrairement aux autres templates Linux embarqués, leur `boot_command` ne saisit pas les identifiants du compte, il se contente de naviguer le menu GRUB et le clavier puis d'attendre que le flux « autoinstall » natif d'Ubuntu se termine de lui-même. Toute autre distribution nécessite un `--template` personnalisé.

Certains templates Linux (`{centos,debian,fedora,opensuse,redhat}.packerlite.yaml`) déclarent désormais aussi un `pre_boot_command:` optionnel, exécuté juste après le démarrage de la VM — avant qu'une IP soit disponible — pour envoyer les frappes de navigation du menu GRUB au tout début du boot, réduisant l'attente aveugle initiale à 5s (au lieu d'environ 30s).
...
| `<reboot>` ou `<reboot stop="true\|false">` | Arrête puis redémarre la VM en cours de provisioning — utile quand l'installeur déclenche lui-même un redémarrage qu'il faut piloter plutôt que subir. `stop="true"` (par défaut) fait un arrêt propre (`requestStop`) ; `stop="false"` force un arrêt immédiat. |
...
`required_variables: [nom1, nom2, ...]` déclare des variables que le template exige : si l'une d'elles manque au moment de la résolution (ni fournie via `--var`, ni via l'éditeur de variables du wizard, ni via les variables intégrées ci-dessous), le provisioning échoue immédiatement avec un message d'erreur explicite plutôt que de substituer silencieusement une chaîne vide. Le template `redhat` l'utilise pour `redhat_username`/`redhat_password` — les identifiants de souscription Red Hat nécessaires à l'enregistrement du système, distincts du compte `--user`/`--password` de la VM.
...
`post_boot_command: { use_ssh_key: true|false, commands: [commande1, commande2, ...] }` déclare une liste de commandes shell exécutées via SSH une fois l'étape d'installation de l'agent terminée (que `install_agent` ait tourné ou non), pratique pour configurer la VM après l'installation — installer des paquets, écrire des fichiers de configuration, lancer un script — sans avoir à le faire via les frappes clavier de `boot_command`. `use_ssh_key` choisit explicitement l'authentification par clé SSH plutôt que par mot de passe pour cette phase, indépendamment de ce que la configuration de la VM fournit par ailleurs. L'exécution s'arrête à la première commande en échec ; par prudence contre une fuite d'identifiants, seuls les journaux de niveau debug affichent le contenu des commandes/sorties — une commande en échec n'est jamais signalée que par son numéro d'ordre. Comme `boot_command`/`pre_boot_command`, les commandes acceptent la même substitution `${var.*}`.
...
Voir `Sources/cakedlib/PackerLite/Resources/*.packerlite.yaml` (tous les templates embarqués — macOS et Linux, avec les titres) pour des exemples complets et entièrement commentés.
...
**Stored provisioning variables**: `--var` (on the CLI — `caked build`/`provision` and `cakectl build`/`provision`) and Caker.app's VM creation wizard "Provisioning variables" editor (shown below the ISO/IPSW image picker) share the same persistent store, `<CAKE_HOME>/ProvisionVariables.json` — a variable added on either side is automatically available on the other, without re-entering it per VM. If no `--var` is passed, the stored variables are used as-is; if a `--var` is passed and one of its keys collides with an already-stored variable of the same name, the command **fails** with a "Duplicate provision variable" error rather than letting either one silently win — rename the variable or remove the conflicting stored entry. Names/values can't contain `=` or `|`.

### Built-in templates

**macOS**: six templates ship as embedded resources (`Sources/cakedlib/PackerLite/Resources/`), one per version — `macos12`, `macos13`, `macos14`, `macos15` (originally transcribed from a Packer `sequoia.pkr.hcl` template), `macos26` (originally transcribed from `tahoe.pkr.hcl`), and `macos27`. Those legacy Packer/Tart reference templates have since been removed from the repo — PackerLite no longer depends on them. All six recognized macOS versions now have a built-in template. Version identifiers were renamed from their original marketing names (`monterey`→`macos12`, `ventura`→`macos13`, `sonoma`→`macos14`, `sequoia`→`macos15`, `tahoe`→`macos26`, `goldengate`→`macos27`); the old names remain accepted as `--macos-version` input for backward compatibility.

**Linux**: nine templates also ship as embedded resources (the `linux-`/`vanilla-` filename prefixes have since been dropped from every bundled template, macOS included): `fedora` (Fedora Workstation, Anaconda — Live ISO, boots into a live desktop and then GNOME's graphical first-boot setup wizard; selected when the URL/filename contains "workstation" or "desktop"), `fedora-server` (Fedora Server, Anaconda — same flow as a CentOS/RHEL ISO, boots straight in with no live session; selected by default otherwise), `centos` (CentOS Stream, Anaconda), `redhat` (RHEL, Anaconda — both `redhat` and `rhel` are recognized in the filename), `opensuse` (openSUSE Leap, YaST installer), `debian` (Debian, debian-installer), `alpine` (Alpine, `setup-alpine`), and `ubuntu`/`ubuntu-server` (Ubuntu Desktop and Server). Most have since been rewritten to use OCR-synchronized (`<locate>`) navigation instead of fixed clicks/delays, across several fix passes explicitly aimed at matching what the real installer actually shows — unlike their original "reviewed for plausibility only" state, that implies iteration against real boots, though no commit claims full end-to-end validation; still expect to need to nudge the occasional click target. `opensuse` targets the YaST installer (openSUSE Leap ≤ 15.6) — the newly-added Leap 16.0/16.1 ISOs in `VMImages.json` use the new Agama installer instead, which this template doesn't cover; bring your own `--template` for those versions in the meantime. Ubuntu also still has its own native unattended-install mechanism (cloud-init/subiquity, via the "autoinstall" boot option) — the `ubuntu*` templates are an optional alternative that goes through the same OCR-driven PackerLite approach every other distro here uses; unlike the other bundled Linux templates, their `boot_command` never types the account credentials, it just navigates the GRUB menu and keyboard layout, then waits for Ubuntu's own autoinstall flow to finish on its own. Any other distro needs a custom `--template`.

Several Linux templates (`{centos,debian,fedora,opensuse,redhat}.packerlite.yaml`) now also declare an optional `pre_boot_command:`, run immediately after the VM starts — before an IP is available — to send GRUB boot-menu keystrokes right at the start of boot, cutting the initial blind wait down to 5s (from roughly 30s).
...
| `<reboot>` or `<reboot stop="true\|false">` | Stops and restarts the VM mid-provisioning-run — useful when the installer itself triggers a reboot that needs to be driven through rather than just survived. `stop="true"` (the default) does a graceful `requestStop`; `stop="false"` forces an immediate stop. |
...
`post_boot_command: { use_ssh_key: true|false, commands: [command1, command2, ...] }` declares a list of shell commands run over SSH once the agent-install step has finished (whether or not `install_agent` actually ran) — handy for configuring the VM after install, such as installing packages, writing config files, or running a setup script, without having to drive it through `boot_command`'s keystrokes. `use_ssh_key` explicitly opts this phase into key-based SSH auth rather than password auth, independent of what the VM's own config provides. Execution stops at the first command that fails; to avoid a credential-leakage risk, only debug-level logs ever show command/output content — a failing command is only ever reported by its position in the list. Like `boot_command`/`pre_boot_command`, these commands support the same `${var.*}` substitution.
...
See `Sources/cakedlib/PackerLite/Resources/*.packerlite.yaml` (every bundled template — macOS and Linux, with titles) for full, fully-commented examples.
