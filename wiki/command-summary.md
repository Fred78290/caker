...
- **Clics assistés par OCR** : maintenez **Fn** enfoncé pendant l'enregistrement pour faire apparaître un surlignage bleu sur chaque texte reconnu à l'écran (reconnaissance Vision). Cliquer dans une zone surlignée enregistre un token `<locate text='...'>` (clic simple) ou `<clickText text='...'>` (Maj+clic) plutôt qu'un `<click point="X,Y">` basé sur des coordonnées fixes — un ancrage qui résiste mieux à un léger décalage de mise en page qu'une coordonnée brute. Un clic en dehors de toute zone surlignée (ou sans Fn) enregistre toujours un `<click point="X,Y">` classique.
...
- **OCR-assisted clicks**: hold **Fn** while recording to highlight every piece of recognized on-screen text in blue (Vision OCR). Clicking inside a highlighted region records a `<locate text='...'>` token (plain click) or `<clickText text='...'>` token (Shift+click) instead of a raw coordinate-based `<click point="X,Y">` — an anchor that's more resilient to small layout shifts than a bare coordinate. A click outside any highlighted region (or without Fn held) still records the usual `<click point="X,Y">`.
