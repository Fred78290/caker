...
`--alias` résout l'identifiant fourni contre le catalogue `VMImages.json` embarqué (le même catalogue que le sélecteur d'images de l'assistant Caker.app) et remplace l'URL/le format de disque par ce que le catalogue indique pour cet identifiant. Pour une entrée `ipsw` dont l'identifiant correspond aussi à une version macOS connue (ex. `macos12`), `--macos-version` est automatiquement renseigné si vous ne l'avez pas précisé vous-même. Chaque identifiant du catalogue est unique : CentOS, openSUSE et Alpine proposent à la fois une ISO d'installation et une image cloud préconstruite, donc leurs entrées `iso` utilisent un identifiant suffixé par `-iso` (ex. `centos10-iso`, `alpine324-iso`) pour rester accessibles à côté de l'identifiant `cloud` correspondant (`centos10`, `alpine324`).
...
`--alias` resolves the given id against the bundled `VMImages.json` catalog (the same catalog behind Caker.app's image picker) and substitutes the URL/disk format the catalog specifies for that id. For an `ipsw` entry whose id also matches a known macOS version (e.g. `macos12`), `--macos-version` is auto-populated if you didn't pass it yourself. Every id in the catalog is unique: CentOS, openSUSE, and Alpine ship both an installer ISO and a prebuilt cloud image, so their `iso` entries use an `-iso`-suffixed id (e.g. `centos10-iso`, `alpine324-iso`) to stay reachable alongside the bare `cloud` id (`centos10`, `alpine324`).
