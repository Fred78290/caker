...
`public-ipv4` / `public-hostname` ne sont disponibles que pour les VM possédant une interface réseau `bridged` (par exemple `cakectl config vm --network name=bridged`) — l'équivalent le plus proche d'une adresse IP publique EC2, puisque le mode bridgé place la VM directement sur le réseau local physique. `public-ipv4` est résolue en priorité via le cache ARP de l'hôte pour l'adresse MAC de cette interface (sollicité par un ping broadcast en cas d'absence d'entrée) — fiable à partir du deuxième démarrage de l'invité (ou dès qu'un bail DHCP est renouvelé), car cloud-init active aussi le sysctl `arp_notify` pour que l'invité annonce lui-même son adresse par ARP gratuit à chaque activation de l'interface ou changement d'adresse ; le noyau ne déclenche cette annonce que sur un événement d'interface, pas rétroactivement au moment où le sysctl est appliqué, donc la toute première adresse obtenue par DHCP au premier démarrage n'en bénéficie pas. Si le cache ARP reste vide malgré tout — sur ce tout premier démarrage, ou en permanence sur le chemin de bridge natif « entitled », où l'hôte ne voit jamais le trafic de l'invité — caker se rabat sur une requête directe à `cakeagent`, à l'intérieur de l'invité, pour ses interfaces réseau. `public-hostname` tente ensuite une résolution DNS inverse (PTR) sur l'adresse ainsi résolue via le résolveur système — ce qui couvre à la fois de vrais enregistrements PTR (si le routeur/DNS du réseau local en publie) et les noms mDNS/Bonjour (`.local`), macOS relayant cette dernière résolution vers `mDNSResponder` — puis se rabat sur `<hostname>.local` si aucune des deux n'aboutit. Sans interface `bridged`, ces deux points de terminaison renvoient 404, comme sur une véritable instance EC2 sans IP publique.
...
`public-ipv4` / `public-hostname` are only available for VMs with a `bridged` network attachment (e.g. `cakectl config vm --network name=bridged`) — the closest analogue to an EC2 public IP this host has, since bridged mode puts the VM directly on the physical LAN. `public-ipv4` is resolved primarily from the host's ARP cache for that interface's MAC address (prodded with a broadcast ping on a cache miss) — reliable from the guest's *second* boot onward (or as soon as a DHCP lease changes), since cloud-init also enables the `arp_notify` sysctl so the guest announces its own address with a gratuitous ARP whenever the interface comes up or its address changes; the kernel only fires that announce on an interface event, not retroactively when the sysctl is applied, so the guest's very first DHCP-acquired address on its first boot isn't covered by it. If the ARP cache still comes up empty — on that first boot, or permanently on the entitled native bridging path, where the host never sees the guest's traffic at all — caker falls back to asking the in-guest `cakeagent` directly for its network interfaces. `public-hostname` then attempts a reverse DNS (PTR) lookup on the resolved address through the system resolver — covering both real PTR records (if the LAN's router/DNS publishes one) and mDNS/Bonjour `.local` names, since macOS transparently routes that lookup through `mDNSResponder` — falling back to `<hostname>.local` if neither resolves. With no `bridged` attachment, both endpoints return 404, matching a real EC2 instance with no public IP.
