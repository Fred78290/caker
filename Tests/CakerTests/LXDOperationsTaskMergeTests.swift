...
final class LXDOperationsTaskMergeTests: XCTestCase {
	...
	func testCancelTaskThroughProviderCancelsTheRegisteredTask() async throws {
		...
		let running = Task {
			_ = try await provider.executeCancellable(command: SleepyCommand(flag: flag), title: "provision my-vm", id: UUID()) {}
		}
	...
	func testListTasksMergesNativeAndRestOperationsExactlyOnce() async throws {
		...
		let running = Task {
			_ = try await provider.executeCancellable(command: SleepyCommand(flag: flag), title: "build native-vm", id: UUID()) {}
		}
