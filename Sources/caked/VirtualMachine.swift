...
final class VirtualMachine: NSObject, VZVirtualMachineDelegate, ObservableObject, VirtioSocketDeviceDelegate {
	...
	func connectionInitiatedByHost(socket: SocketDevice) {
		...
		/*if socket.bind == self.vmLocation.agentURL.path {
			...
			do {
				try PortForwardingServer.createPortForwardingServer(group: Root.group.next(), remoteAddress: self.runningIP, forwardedPorts: self.config.forwardedPorts, dynamicPortFarwarding: config.dynamicPortFarwarding, listeningAddress: self.vmLocation.agentURL, asSystem: asSystem)
			} catch {
				Logger(self).error(error)
			}
		}*/
