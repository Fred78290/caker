extension IP {
	...
	public struct V4: Address, ExpressibleByIntegerLiteral {
		...
		@inlinable public init(storage: UInt32) {
...
extension IP.V4 {
	@inlinable public init(_ a: UInt8, _ b: UInt8, _ c: UInt8, _ d: UInt8) {
...
extension IP.V4 {
	...
	/// loopback range.
	@inlinable public static var localhost: Self {
		.init(value: 0x7F_00_00_01)
	}
}
...
extension IP.V4: LosslessStringConvertible {
	...
	@inlinable public init?(_ description: some StringProtocol) {
		guard var b: String.Index = description.firstIndex(of: "."), let a: UInt8 = .init(description[..<b]) else {
			return nil
		}

		b = description.index(after: b)

		guard var c: String.Index = description[b...].firstIndex(of: "."), let b: UInt8 = .init(description[b..<c]) else {
			return nil
		}

		c = description.index(after: c)

		guard var d: String.Index = description[c...].firstIndex(of: "."), let c: UInt8 = .init(description[c..<d]) else {
			return nil
		}

		d = description.index(after: d)

		guard let d: UInt8 = .init(description[d...]) else {
			return nil
		}
