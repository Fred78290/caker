extension IP {
	...
	public protocol Address: LosslessStringConvertible, Comparable, Equatable, Hashable, Sendable {
...
extension IP.Address {
	...
	/// hextet.
	@inlinable public init(value: Storage) {
	...
	@inlinable public var value: Storage {
		.init(bigEndian: self.storage)
	}
}

extension IP.Address where Self: ExpressibleByIntegerLiteral, Self.IntegerLiteralType == Storage {
	@inlinable public init(integerLiteral: Storage) {
		self.init(value: integerLiteral)
	}
}
...
extension IP.Address {
	...
	@inlinable static var bitWidth: UInt8 {
		.init(Storage.bitWidth)
	}
	...
	@inlinable static func storageMask(ones bits: UInt8) -> Storage {
		let ones: Storage = ~0
	...
	@inlinable public func zeroMasked(to bits: UInt8) -> Self {
	...
	@inlinable public func onesMasked(to bits: UInt8) -> Self {
		.init(storage: self.storage | ~Self.storageMask(ones: bits))
	}
	...
	@inlinable public static func / (self: Self, bits: UInt8) -> IP.Block<Self> {
