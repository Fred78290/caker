...
struct CurrentStatusHandler: CakedCommandAsync {
	...
	mutating func run(on: EventLoop, runMode: Utils.RunMode) async -> Caked_Reply {
		do {
			_ = try await CakedLib.CurrentStatusHandler.currentStatus(vmname: self.request.name, frequency: self.request.frequency, responseStream: self.responseStream, runMode: runMode)
		} catch {
