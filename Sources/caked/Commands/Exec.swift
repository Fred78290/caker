...
struct Exec: CakeAgentAsyncParsableCommand {	
	...
	func run(on: EventLoopGroup, helper: CakeAgentHelper, callOptions: CallOptions?) async throws {
		guard let result = try? CakedLib.StartHandler.startVM(name: self.execute.name, screenSize: nil, vncPassword: nil, vncPort: nil, waitIPTimeout: self.execute.waitIPTimeout, startMode: self.execute.foreground ? .foreground : .background, gcd: false, recoveryMode: false, runMode: self.common.runMode) else {
		...
		if result.started {
			...
			do {
				let exitCode = try await helper.exec(command: command, arguments: arguments, callOptions: callOptions)
