...
struct Mount: ParsableCommand {
	...
	func run() throws {
		...
		Logger.appendNewLine(self.common.format.render(CakedLib.MountHandler.Mount(mode, location: location, mounts: self.mount.mounts, runMode: self.common.runMode)))
	}
}
