...
struct Build: AsyncParsableCommand {
	...
	func run() async throws {
		try await CakedLib.BuildHandler.build(name: self.options.name, options: self.options, runMode: self.common.runMode, progressHandler: CakedLib.BuildHandler.progressHandler)
	}
}
