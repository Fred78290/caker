extension IP {
	...
	@frozen public struct Block<Base>: Equatable, Hashable, Sendable where Base: Address {
		public var base: Base
		public var bits: UInt8

		@inlinable
		public init(base: Base, bits: UInt8) {
			self.base = base
			self.bits = bits
		}
	}
...
extension IP.Block {
	@inlinable public func contains(_ ip: Base) -> Bool {
		self.base == ip.zeroMasked(to: self.bits)
	}
...
extension IP.Block: LosslessStringConvertible {
	...
	@inlinable public init?(_ string: some StringProtocol) {
		guard let slash: String.Index = string.lastIndex(of: "/"), let base: Base = .init(string[..<slash]), let bits: UInt8 = .init(string[string.index(after: slash)...]), bits <= Base.bitWidth else {
			return nil
		}
