...
final class ConsoleDevice: CatchRemoteCloseDelegate {
	...
	private func createUnixConsole(consoleURL: URL) -> (FileHandle, FileHandle) {
		...
		let binder = bootstrap.bind(unixDomainSocketPath: consoleURL.path(percentEncoded: false), cleanupExistingSocketFile: true)
	...
	private func createFileConsole(consoleURL: URL) throws -> (FileHandle, FileHandle) {
		if FileManager.default.fileExists(atPath: consoleURL.absoluteURL.path(percentEncoded: false)) {
			try FileManager.default.removeItem(at: consoleURL)
		}

		FileManager.default.createFile(atPath: consoleURL.absoluteURL.path(percentEncoded: false), contents: nil)
