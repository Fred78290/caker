...
extension SPICEViewServer {
    ...
    public static func forGLView(_ glView: NSView,
                               spiceConfig: SPICEServer.Configuration,
                               captureConfig: CaptureConfiguration = .performance) -> SPICEViewServer {
        let server = SPICEViewServer(sourceView: glView, 
    ...
    public static func forContentView(_ view: NSView,
                                    spiceConfig: SPICEServer.Configuration,
                                    captureConfig: CaptureConfiguration = .quality) -> SPICEViewServer {
        return SPICEViewServer(sourceView: view,
                             spiceConfiguration: spiceConfig,
                             captureConfiguration: captureConfig)
    }
    
    /// Crée un serveur SPICE pour une NSWindow complète
    public static func forWindow(_ window: NSWindow,
                               spiceConfig: SPICEServer.Configuration,
                               captureConfig: CaptureConfiguration = .balanced) -> SPICEViewServer {
        return SPICEViewServer(sourceView: window.contentView!,
                             spiceConfiguration: spiceConfig,
                             captureConfiguration: captureConfig)
    }
}
