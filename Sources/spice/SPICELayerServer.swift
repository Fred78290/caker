...
extension SPICELayerServer {
	...
	public static func forOpenGLLayer(
		_ openGLLayer: CAOpenGLLayer,
		spiceConfig: SPICEServer.Configuration,
		captureConfig: CaptureConfiguration = .balanced
	) -> SPICELayerServer {
		return SPICELayerServer(
			sourceLayer: openGLLayer,
			spiceConfiguration: spiceConfig,
			captureConfiguration: captureConfig)
	}

	/// Crée un serveur SPICE pour un layer de contenu générique
	public static func forContentLayer(
		_ layer: CALayer,
		spiceConfig: SPICEServer.Configuration,
