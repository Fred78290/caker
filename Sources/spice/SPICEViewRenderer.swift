...
public class SPICEViewRenderer {
    ...
    private func renderViewWithMetal(_ view: NSView) -> Data? {
        guard let _ = metalDevice,
              let commandQueue = metalCommandQueue else { return nil }
        
        let bounds = view.bounds
        updateRenderTargetSize(CGSize(width: bounds.width * configuration.scaleFactor,
    ...
    private func updateRenderTargetSize(_ size: CGSize) {
        guard useMetalAcceleration,
              ...
              let currentTexture = renderTargetTexture else { return }
