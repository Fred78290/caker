...
public class SPICEManager {
    ...
    public var isHealthy: Bool {
        guard isActive,
              ...
              server.running else { return false }
        
        return true
    }
    
    /// Génère une configuration SPICE optimisée
    public static func optimizedConfiguration(port: Int, 
                                            password: String? = nil,
                                            quality: Quality = .balanced) -> SPICEServer.Configuration {
        switch quality {
        case .performance:
...
extension SPICEManager {
    ...
    public static func createVirtualMachine(with configuration: VZVirtualMachineConfiguration,
                                          spiceConfiguration: SPICEServer.Configuration) throws -> (VZVirtualMachine, SPICEManager) {
    ...
    public func configureForVirtualMachine(_ virtualMachine: VZVirtualMachine,
                                         autoStart: Bool = true) {
        self.virtualMachine = virtualMachine
...
extension SPICEManager {
    ...
    public func testConnectivity(timeout: TimeInterval = 5.0, completion: @escaping (Bool, Error?) -> Void) {
        ...
        let client = SPICEClient(host: "127.0.0.1", 
                               port: configuration.port, 
