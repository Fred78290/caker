...
public struct Utils {
	...
	public enum RunMode: Sendable, Codable {
		case system
		case user
		case app
	...
	public static func getHome(runMode: RunMode, createItIfNotExists: Bool = true) throws -> URL {
		guard let cakeHomeDir = homeDirectories[runMode.isSystem] else {
			...
			if let customHome = ProcessInfo.processInfo.environment["CAKE_HOME"] {
				...
				cakeHomeDir = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.\(cakerSignature)")!
				//cakeHomeDir = FileManager.realHomeDirectoryForCurrentUser.appendingPathComponent("Library/Group Containers/\(cakerSignature)", isDirectory: true)
...
public func withAsyncResult<T>(builder: @escaping () async throws -> T) throws -> T {
	let semaphore = DispatchSemaphore(value: 0)
	let pointer = UnsafeMutablePointer<T>.allocate(capacity: 1)
	let err = UnsafeMutablePointer<Error?>.allocate(capacity: 1)
